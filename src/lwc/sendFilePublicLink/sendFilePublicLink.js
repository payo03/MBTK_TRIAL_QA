...
export default class SendFilePublicLink extends NavigationMixin(LightningElement) {
  ...
  handleSend() {
    ...
    if (!this.selectedValue) {
      showToast("", "견적서를 발송할 언어를 선택해주세요.", "warning");
    } else {
      this.isLoading = true;
      createPublicLink({inputMap: inputMap}).then(res => {
        console.log("res :: ", res);
        if (res === null) {
          title = "오류";
          msg = "관리자에게 문의해주세요."
  ...
  handleSave() {
    ...
    if (!this.selectedValue) {
      showToast("", "견적서를 저장할 언어를 선택해주세요.", "warning");
    } else {
      this.isLoading = true;
      createPublicLink({inputMap: inputMap}).then(res => {
        console.log("res :: ", res);
        if (res === null) {
          ...
          //   recordNavigation(this, "Quote", this.recordId, null, false);
          // });
        };
        this.onRefresh();
        showToast(title, msg, variant);
